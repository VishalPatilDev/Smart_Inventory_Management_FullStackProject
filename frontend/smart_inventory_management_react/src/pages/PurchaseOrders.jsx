// src/pages/PurchaseOrders.jsx — updated with toast and field errors
...
export default function PurchaseOrders() {
  ...
  const [form, setForm] = useState({ supplierId: '', warehouseId: '', items: [{ productId: '', quantity: '', price: '' }] })
  ...
  const handleSave = () => handle(
    ...
    `Purchase order created — ${form.items.length} item(s) added to stock`,
    () => {
      setModal(false)
      setForm({ supplierId: '', warehouseId: '', items: [{ productId: '', quantity: '', price: '' }] })
      refetch()
    }
  )
  ...
  return (
    <div>
      <PageHeader
        ...
        action={<Button onClick={() => { clearErrors(); setModal(true) }}>+ New Purchase Order</Button>}
      />
      ...
      {modal && (
        <Modal title="New Purchase Order" onClose={() => setModal(false)} width={600}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            ...
            <FormField label="Receive into Warehouse" required>
          ...
          {form.items.map((item, i) => (
            <div key={i} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr auto', gap: 8, marginBottom: 8, alignItems: 'end' }}>
              ...
              <div>
                {i === 0 && <div style={{ fontSize: 11, color: 'var(--text-3)', marginBottom: 4 }}>Unit Price ₹</div>}
                <Input type="number" min="0" value={item.price} onChange={(e) => updateItem(i, 'price', e.target.value)} placeholder="0" />
              </div>
          ...
          <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 16 }}>
            ...
            <Button onClick={handleSave} disabled={saving}>{saving ? 'Creating…' : 'Create Purchase Order'}</Button>
          </div>
        </Modal>
      )}
    </div>
